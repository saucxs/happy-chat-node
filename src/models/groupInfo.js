...
// 建群
let createGroup = (arr) => {
	let _sql = "INSERT INTO group_info (group_id,group_name,group_notice,group_avatar,group_creater,creater_time) VALUES (?,?,?,?,?,?)"
...
let getGroups = function(user_id) {
    const _sql =
        "SELECT gur.group_id , gur.user_id , g.group_name , g.group_notice , g.group_avatar, g.group_creater , g.creater_time FROM (select * from group_user_relation order by id desc) as gur  inner join  group_info as g on gur.group_id = g.group_id  WHERE  gur.user_id = ?";
